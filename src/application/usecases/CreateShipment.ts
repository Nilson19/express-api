import { IShipmentRepository } from "../../domain/repositories/IShipmenRepository";
import { Shipment } from "../../domain/entities/Shipment";

export class CreateShipment {
  constructor(private shipmentRepository: IShipmentRepository) {}

  async execute(data: Shipment): Promise<string> {
    // Validar los datos de entrada
    if (!data.origin_zip || !data.destination_zip || !data.weight) {
      throw new Error("Datos de envío incompletos");
    }

    // Crear el envío
    const shipmentId = await this.shipmentRepository.createShipment(data);

    return shipmentId; // Retorna el ID del envío creado
  }
}