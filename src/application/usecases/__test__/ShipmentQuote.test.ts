...
describe("ShipmentQuote", () => {
    ...
    it("should return the cost when valid parameters are provided", async () => {
        ...
        const result = await shipmentQuote.getShipmentQuote("110111", "220222", 5);
        expect(result).toBe("100.00");
        expect(tariffRepository.getTariff).toHaveBeenCalledWith("110111", "220222", 5);
    });

    it("should throw an error if originZip is missing", async () => {
        await expect(
            shipmentQuote.getShipmentQuote("", "220222", 5)
        ).rejects.toThrow("Invalid input parameters for shipment quote.");
    });

    it("should throw an error if destinationZip is missing", async () => {
        await expect(
            shipmentQuote.getShipmentQuote("110111", "", 5)
        ).rejects.toThrow("Invalid input parameters for shipment quote.");
    });

    it("should throw an error if weight is zero or negative", async () => {
        await expect(
            shipmentQuote.getShipmentQuote("110111", "220222", 0)
        ).rejects.toThrow("Invalid input parameters for shipment quote.");

        await expect(
            shipmentQuote.getShipmentQuote("110111", "220222", -1)
        ).rejects.toThrow("Invalid input parameters for shipment quote.");
    });
    ...
    it("should throw an error if no tariff is found", async () => {
        ...
        await expect(
            shipmentQuote.getShipmentQuote("110111", "220222", 5)
